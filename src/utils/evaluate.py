...
def run_evaluation(args, inputs, refs, evaluator):
    ...
    r = (
        f"{args.input_path}|{result['accuracy']:.3f}|{result['similarity']:.3f}|{result['fluency']:.3f}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="../../experiments/results", help="Directory where to save the results",
    )

    parser.add_argument(
        "--source_list",
        type=str,
        help="path to the `test_toxic_parallel.txt` file",
        default="data/test_toxic_parallel.txt"
    )

    parser.add_argument(
        "--references_list",
        type=str,
        help="path to the `test_neutral_parallel.txt` file",
        default="data/test_neutral_parallel.txt"
    )
    ...
    with open(args.references_list, 'r') as f:
        refs = [line.strip() for line in f.readlines()]

    run_evaluation(args, inputs, refs, evaluator=evaluate)
